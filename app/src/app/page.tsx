...
import { MediationFunnel } from "@/components/MediationFunnel";
import { EvidenceNetwork } from "@/components/EvidenceNetwork";
import { AcknowledgementsBlock } from "@/components/AcknowledgementsBlock";
import { useRef } from "react";
import { FileText, ShieldAlert, Users, Scale, ExternalLink, Gavel } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main ref={containerRef} className="min-h-screen relative pb-32">
      ...
      <div className="fixed top-20 right-5 z-50 flex flex-col gap-3 items-end">
        <a
          href="/brief"
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#d4af37] text-black text-sm font-bold shadow-[0_0_20px_rgba(212,175,55,0.6)] hover:shadow-[0_0_35px_rgba(212,175,55,0.9)] hover:scale-105 transition-all duration-300"
        >
          <Gavel className="w-4 h-4" />
          Brief for Counsel
        </a>
        <a
          href="/letters_visual.html"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-4 py-2 rounded-full border border-[#d4af37]/50 text-[#d4af37] bg-black/60 backdrop-blur-md text-sm font-bold hover:bg-[#d4af37] hover:text-black hover:border-[#d4af37] transition-all duration-300 shadow-lg hover:shadow-[0_0_20px_rgba(212,175,55,0.4)]"
        >
      ...
      <MediationFunnel lang={lang} />
      <MediationWall data={t.wall} lang={lang} />
      <PatternHeatmap lang={lang} />
      <PatternOfProvocation lang={lang} />
      <EvidenceNetwork lang={lang} />
      <Timeline lang={lang} />
      <UndatedRecords lang={lang} />
      <Incidents lang={lang} />
      <AcknowledgementsBlock lang={lang} />
      ...
      {/* Action CTAs */}
      <section className="relative z-10 mt-24 px-6 pb-24 flex flex-col gap-8">
        <div className="max-w-5xl mx-auto w-full">
          ...
          <motion.a
            ...
            className="group relative flex flex-col md:flex-row items-center justify-between gap-8 p-10 md:p-14 rounded-3xl overflow-hidden cursor-pointer border border-[#d4af37]/40 hover:border-[#d4af37] transition-all duration-500 mb-8"
            style={{ background: "linear-gradient(135deg, #121820 0%, #0a0e14 50%, #121820 100%)" }}
          >
            {/* Glow effects */}
            <div className="absolute inset-0 pointer-events-none">
              ...
              <div className="absolute bottom-0 left-1/4 w-1/2 h-px bg-gradient-to-r from-transparent via-[#d4af37]/30 to-transparent" />
            ...
            <div className="relative text-center md:text-left">
              ...
              <h3 className="text-3xl md:text-5xl font-bold text-white mb-4 group-hover:text-[#d4af37] transition-colors duration-300 leading-tight">
                Visual Timeline Archive
              </h3>
              <p className="text-neutral-400 leading-relaxed max-w-xl">
                Explore the chronological record of 23 handwritten letters, pattern analysis, and the 10-year history of the conflict in a visual format.
              </p>
            </div>

            <div className="relative shrink-0 flex flex-col items-center gap-3">
              <div className="w-20 h-20 rounded-full border-2 border-[#d4af37] bg-[#d4af37]/10 flex items-center justify-center group-hover:bg-[#d4af37] transition-all duration-300 shadow-[0_0_30px_rgba(212,175,55,0.2)] group-hover:shadow-[0_0_50px_rgba(212,175,55,0.5)]">
                <ExternalLink className="w-8 h-8 text-[#d4af37] group-hover:text-black transition-colors duration-300" />
              </div>
              <span className="text-[#d4af37] text-sm font-semibold tracking-wide">View Timeline</span>
            </div>
          </motion.a>
